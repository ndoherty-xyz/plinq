...
function playLayer(
  ...
  if (layer.source.type === 'oscillator') {
    ...
    if (layer.source.frequencyEnd != null) {
      ...
      if (curve === 'exponential') {
        osc.frequency.exponentialRampToValueAtTime(Math.max(endFreq, 0.001), rampEnd)
      } else {
  ...
  if (layer.filter) {
    ...
    filterNode.frequency.setValueAtTime(layer.filter.frequency * pitchMul, startTime)
  ...
  if (curve === 'exponential') {
    gainNode.gain.exponentialRampToValueAtTime(Math.max(peakGain, 0.001), startTime + layer.envelope.attack)
    gainNode.gain.exponentialRampToValueAtTime(0.001, decayEnd)
  } else {
    gainNode.gain.linearRampToValueAtTime(peakGain, startTime + layer.envelope.attack)
